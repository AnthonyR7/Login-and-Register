...
def process():
    ...
    if len(request.form['f_name']) < 2 :
        ...
        hashed_pass = md5.new(request.form['pass_word']).hexdigest()
        hasted_confirm = md5.new(request.form['confirm_password']).hexdigest()
        query = "INSERT INTO users (first_name,last_name,email,pass_word) VALUES (:f_name,l_name,email,hashed_pass)"
        data = {
                'first_name':request.form['f_name'],
                'last_name':request.form['l_name'],
                'email': request.form['email'],
                'pass_word':hasted_pass
                }
